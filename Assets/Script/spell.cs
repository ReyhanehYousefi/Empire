...
public class spell : MonoBehaviour
{
    ...
    private GameObject doorInstance; // Reference to the instantiated exit door
    public Vector2 loc;
    private void Start()
    {
    ...
    private void OnMouseDown()
    {
        if (!isCardSelected && Input.GetMouseButtonDown(0))
        {
            if (Map.cont)
            {
                ...
                if (isCardAdjacent)
                {
                    ...
                    doorInstance = Instantiate(exitdoor, loc, Quaternion.identity);
                }
            }
        }
        else if (isCardSelected && Input.GetMouseButtonDown(0))
        {
            ...
            if (pos.x == transform.position.x && pos.y == transform.position.y)
            {
                ...
                // Disable and destroy the exit door
